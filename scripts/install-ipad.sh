...
DERIVED_DATA_PATH="${SNIPPETS_IPAD_DERIVED_DATA:-/tmp/snippets-ipad-device-derived}"
...
function usage() {
    cat <<'EOF'
Build, validate, install, and launch Snippets on a connected iPad.

Usage:
  ./scripts/install-ipad.sh [options]

Options:
  --device <name>       Select an iPad when more than one is paired.
...
Environment:
  SNIPPETS_IPAD_DERIVED_DATA  Default derived-data directory override.
...
# Returns 2 when SpringBoard rejected the launch only because the iPad is locked.
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/snippets-ipad-install.XXXXXX")"
trap 'rm -rf "$WORK_DIR"' EXIT

info "Discovering paired iPads"
...
if [ -z "$DEVICE_SELECTOR" ] && [ "$available_count" -gt 1 ]; then
    printf 'More than one paired iPad is available:%b\n' "$available_names" >&2
    fail "Select one with --device <name>"
fi
if [ "$matched_count" -eq 0 ]; then
    if [ "$available_count" -gt 0 ]; then
        printf 'Available paired iPads:%b\n' "$available_names" >&2
    fi
    fail "No matching paired iPad is available"
fi
if [ "$matched_count" -gt 1 ]; then
    fail "The device selector matched more than one iPad; use an exact name"
fi

if [ -n "$DEVICE_OS_VERSION" ]; then
    success "$DEVICE_NAME ($DEVICE_MODEL, iPadOS $DEVICE_OS_VERSION)"
else
...
if [ -z "$xcode_destination_line" ] || [[ "$xcode_destination_line" == *"error:"* ]]; then
    fail "The paired iPad is not an eligible destination for the $SCHEME scheme"
fi
success "Eligible native iPad destination"
...
if [ "$provisions_all_devices" != "true" ] \
    ...
    fail "The provisioning profile does not include the selected iPad"
fi
...
if [ "$LAUNCH_APP" -eq 1 ]; then
    ...
    if [ "$launch_status" -eq 2 ] && [ -t 0 ]; then
        warn "The iPad is locked. Unlock it, then press Return to retry the launch."
        read -r
        launch_status=0
        launch_app_once || launch_status=$?
    fi
    ...
    if [ "$launch_status" -eq 0 ]; then
        ...
        warn "Installed, but the iPad is locked. Unlock it and rerun with --no-build."
    else
