...
| Clipboard managers | Command-Backslash Secure Paste never puts the rendered body on the pasteboard or creates a delivery lease: it preselects one Accessibility route, or a bounded targeted-Unicode route for ordinary content only, and never falls back after a write attempt. An ordinary `{clipboard}` placeholder may still read the user's current clipboard, and startup may finish restoring an older lease. A secure body is refused when only multi-event Chromium delivery is available. Authenticated trigger expansion still prefers Accessibility; where its legacy event path makes the pasteboard unavoidable, `TemporaryPasteboardLease(isConcealed:)` sets `org.nspasteboard.ConcealedType` and `TransientType` — a **courtesy, not a control**: there is no AppKit constant, managers honour it only by convention, and anything that ignores it still sees the text. |
