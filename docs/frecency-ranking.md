...
| авто-раскрытие после свежего AX-confirmation | `refreshSuggestionContextFromFocusedText` → `selectSuggestion` | 1.0 | ✔ | ✘ |
...
`acceptSelectedSuggestion(_:)` делает одно немедленное ограниченное по времени точное AX-чтение — без sleep и без локального fallback — и **может прерваться без раскрытия**, если trigger отсутствует, изменился, недоступен или содержит небезопасную для подсчёта графему. После подтверждения очередь всё ещё может отказаться от вставки, если сменились приложение, фокус или поколение контекста. Счёт в момент нажатия засчитывал бы каждое такое прерывание.
...
| `usage.record` в `expand()` после AX-confirmation | **нет** | следующий main-loop `Task` или callback `AXObserver`, оба выполняются вне callback'а `CGEvent`-тапа |
...
1. Синхронное локальное обновление и любое последующее обновление по AX-notification ранжируют по одному замороженному снимку. Строки не перетасовываются под пальцами.
...
2. **Стабильность сессии под пальцами.** В TextEdit и в Chromium-приложении набрать `\` и затем запрос посимвольно; убедиться, что ни одна строка не двигается иначе как вследствие изменения запроса. Конкретно — что локальная перерисовка и последующие AX-notification не перетасовывают список.
