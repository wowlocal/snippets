...
Пустой запрос на неделе 6 **в текущем коде** дал бы `[lgtm, mig, …]`: `snippetsSortedForDisplay()` (`SnippetStore.swift:220-224`) сохраняет порядок массива, а `addSnippet` вставляет в индекс 0 (`SnippetStore.swift:97`), то есть новейшее — первым; `lgtm` создан на неделе 3, `mig` — на неделе 0. С frecency: `sig, lgtm, addr, mig, rma`.
...
* **`SnippetStore.snippetsSortedForDisplay()` (`:220-224`)** — ноль строк. Соблазнительная однострочная правка и самая опасная: это общий базовый порядок и для основного списка (`ViewController+State.swift:44`), и для панели (`SnippetExpansionEngine.swift:851`). Одна строка переупорядочила бы вручную собранную библиотеку под анимированным diff'ом (`applyAnimatedListUpdate`, `ViewController+State.swift:114-138`, который выше 40 изменений падает в `reloadData()`) **и** молча переопределила бы tie-break `displayOrder` в панели — два изменения поведения, которые нельзя откатить по отдельности.
...
**1. Только что созданный сниппет тонет в ветке пустого запроса.** `addSnippet` вставляет в индекс 0 (`SnippetStore.swift:97`), поэтому сегодня свежесозданный сниппет — первая строка панели при пустом запросе. С frecency выше `displayOrder` он уходит ниже любого сниппета с ненулевой историей — то есть именно тот, чьё ключевое слово пользователь ещё не запомнил, становится труднее всего найти.
